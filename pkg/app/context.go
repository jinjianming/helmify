...
func (c *appContext) CreateHelm(stop <-chan struct{}) error {
	...
	return c.output.Create(c.config.ChartDir, c.config.ChartName, c.config.ChartVersion, c.config.Crd, c.config.CertManagerAsSubchart, c.config.CertManagerVersion, templates, filenames)
}
